...
class ConsistentHashRing:
    ...
    def add_newRequest(self, request):
        ...
        if self.get_alive_servers()>0:
            ...
            self.requestHistory[request]=key
            return end-start
        else:
            return False
...
def visualization_from_dataset(total_nodes, num_servers, server_capacity, all_requests, threshold):
    ...
    for server in ring.servers:
        ...
        if isinstance(server, Server):
            ...
            figure=os.path.join(currDir,"ConsistentHistoryOutputs",f"CHBaseline-HHFS{server.name}_sNum{serverNum}_cap{server_capacity},thold{threshold}.png")
            plt.savefig(figure)
            # plt.show()
    ...
    plt.legend()
    figure=os.path.join(currDir,"ConsistentHistoryOutputs",f"CHBaseline-SLOT_sNum{serverNum}_cap{server_capacity},thold{threshold}.png")
    plt.savefig(figure)
    # plt.show()
    ...
    plt.legend()
    figure=os.path.join(currDir,"ConsistentHistoryOutputs",f"CHBaseline-SHSOI_sNum{serverNum}_cap{server_capacity},thold{threshold}.png")
    plt.savefig(figure)
    # plt.show()
    ...
    figure=os.path.join(currDir,"ConsistentHistoryOutputs",f"SHSHOI_sNum{serverNum}_cap{server_capacity},thold{threshold}.png")
    plt.savefig(figure)
    # plt.show()
    ...
    plt.grid(True)
    figure=os.path.join(currDir,"ConsistentHistoryOutputs",f"TVRI_sNum{serverNum}_cap{server_capacity},thold{threshold}.png")
    plt.savefig(figure)
    # plt.show()
    ...
    ax.legend()
    figure=os.path.join(currDir,"ConsistentHistoryOutputs",f"CHBaseline-COHHAIHFES_sNum{serverNum}_cap{server_capacity},thold{threshold}.png")
    plt.savefig(figure)
    # plt.show()
